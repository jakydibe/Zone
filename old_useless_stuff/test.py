...
#relocate_image()
    def write_pe_text_section(self):
        ...
        with open("hello_world_patched.exe", "r+b") as f:
            ...
            addr = 0x1000
            modified_file = modified_file[:0x120] + addr.to_bytes(4, byteorder='little') + modified_file[0x124:]
        ...
        for instr in self.new_instructions:
            if instr.new_instr.address > starting_addr:
                ...
                for i in self.cs.disasm(new_bytes, (instr.new_instr.address + num_bytes)):
                    ...
                    instr.new_instr = i
                #per aggiustare le references alla .data section
                if 'rip + ' in instr.new_instr.op_str:
                    ...
                    if old_address != new_address:
                        ...
                        new_bytes = bytearray(asm)
                        #se l' assembler fallisce miseramente
                        if len(new_bytes) != instr.new_instr.size:
                            # print("ERRORE: ", hex(instr.new_instr.address))
                            # print("SIZE OLD: ", instr.new_instr.size)
                            # print("SIZE NEW: ", len(new_bytes))
                            # print("I: ", instr.new_instr.mnemonic)
                            # print("I: ", instr.new_instr.op_str)
                            # print("I: ", instr.new_instr.bytes)
                            # print("I: ", instr.new_instr.size)
                            # print("I: ", instr.new_instr.address)
                            # print("`nuova` istruzione: ", string)
                            # print("`nuovi` bytes: ", new_bytes)
                            new_bytes.insert(0,instr.new_instr.bytes[0])
                        for n in self.cs.disasm(new_bytes, instr.new_instr.address):
                            instr.new_instr = n
                        
            #sommo num_bytes alle istruzioni dei jump

    def insert_instruction(self, i,position):
        previous_instr = self.new_instructions[position -1]

        new_instr = Instruction(i,i,i,previous_instr,None)

        self.new_instructions.insert(position,new_instr)
        ...
        for label in self.label_table:
            for instr in self.new_instructions:
                ...
                if instr.old_instr.address == label.label_address:
                    ...
                    #skippo se l' indirizzo di jump e' fuori dalla .text section
                    if label.label_address > (self.code_section.VirtualAddress + self.code_section.SizeOfRawData):
                        continue
                    
                    #skippo se e' una chiamata tipo call qword ptr [rip + 0x10dd], ovvero tipo una chiamata a func esterne
                    if 'rip +' in label.instr.new_instr.op_str:
                        continue
                    ...
                    if len(new_bytes) != label.instr.new_instr.size:
                        print("ERRORE: ", hex(label.instr.new_instr.address))
                        print("SIZE OLD: ", label.instr.new_instr.size)
                        print("SIZE NEW: ", len(new_bytes))
                        print("I: ", label.instr.new_instr.mnemonic)
                        print("I: ", label.instr.new_instr.op_str)
                        print("I: ", label.instr.new_instr.bytes)
                        print("I: ", label.instr.new_instr.size)
                        print("I: ", label.instr.new_instr.address)
                        print("`nuova` istruzione: ", str_instr)
                        print("`nuovi` bytes: ", new_bytes)
                        if (label.instr.new_instr.size > len(new_bytes)):
                            nop_num = len(label.instr.new_instr.bytes) - len(new_bytes)
                            for i in range(nop_num):
                                new_bytes.append(0x90)
                        
                            
                            for num_instr,i in enumerate(self.new_instructions):
                                if i.new_instr.address == label.instr.new_instr.address:
                                    prima_instr = self.new_instructions[num_instr]
                                    for x,n in enumerate(self.cs.disasm(new_bytes, i.new_instr.address)):
                                        if x == 0:
                                            i.new_instr = n
                                        elif x == (nop_num):
                                            previous_instr = self.new_instructions[num_instr + x - 1].new_instr
                                            new_instr = Instruction(n,n,n,previous_instr,prima_instr.next_instr)
                                            previous_instr.next_instr = new_instr
                                            self.new_instructions.insert(num_instr + x,new_instr)                                     
                                        else:
                                            previous_instr = self.new_instructions[num_instr + x - 1].new_instr
                                            new_instr = Instruction(n,n,n,previous_instr,None)
                                            previous_instr.next_instr = new_instr

                                            self.new_instructions.insert(num_instr + x,new_instr)
                                        print("instr: ", n.mnemonic, n.op_str, hex(n.address))
                                    break

                        #new_bytes.insert(0,label.instr.new_instr.bytes[0])
                        #exit(1)
                        continue
